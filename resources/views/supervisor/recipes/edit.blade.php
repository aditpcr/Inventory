...
<div class="flex items-center space-x-2 text-sm">
    ...
    <a href="{{ route('supervisor.recipes.show', $recipe) }}" class="text-blue-600 hover:text-blue-700 font-medium">{{ $recipe->name }}</a>
    <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-3 mb-2">
            ...
            <h1 class="text-2xl font-black text-gray-900">Edit {{ $recipe->name }}</h1>
        </div>
        <p class="text-gray-600">Update recipe details and ingredients</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200/60 backdrop-blur-sm bg-white/90">
        <div class="px-6 py-5 border-b border-gray-200/60">
            <h2 class="text-lg font-bold text-gray-900">Edit Recipe Information</h2>
        </div>
        
        <form action="{{ route('supervisor.recipes.update', $recipe) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $recipe->name) }}" 
                ...
                <div>
                    ...
                    <select name="menu_item_id" id="menu_item_id" 
                        ...
                        <option value="{{ $menuItem->id }}" {{ old('menu_item_id', $recipe->menu_item_id) == $menuItem->id ? 'selected' : '' }}>
                            {{ $menuItem->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              ...
                              placeholder="Enter recipe description, instructions, or notes...">{{ old('description', $recipe->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        </p>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 mt-6 border-t border-gray-200">
                <a href="{{ route('supervisor.recipes.show', $recipe) }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addIngredientField('{{ $ingredient->id }}', '{{ $ingredient->pivot->quantity }}');
    @endforeach

    // Add initial field if no ingredients
