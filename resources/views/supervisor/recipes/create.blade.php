...
<div class="container" style="max-width: 1000px;">
    ...
    <div class="card">
        ...
        <form action="{{ route('supervisor.recipes.store') }}" method="POST" class="card-body">
            @csrf
            
            <div style="display: flex; flex-direction: column; gap: var(--space-6);">
                <!-- Menu Item -->
                <div>
                    <label for="menu_item_id" class="form-label flex items-center">
                        <i class="fas fa-utensils" style="margin-right: var(--space-2); color: var(--accent-color);"></i>Associated Menu Item *
                    </label>
                    <select name="menu_item_id" id="menu_item_id" class="form-input" required>
                        <option value="">Select Menu Item</option>
                        @foreach($menuItems as $menuItem)
                        <option value="{{ $menuItem->id }}" {{ old('menu_item_id', request('menu_item_id')) == $menuItem->id ? 'selected' : '' }}>
                            {{ $menuItem->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="quantity_needed" id="quantity_needed" value="{{ old('quantity_needed') }}" 
                           class="form-input"
            ...
            <div class="flex items-center justify-end" style="gap: var(--space-4); padding-top: var(--space-6); margin-top: var(--space-6); border-top: 1px solid var(--border-light);">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save" style="margin-right: var(--space-2);"></i>Create Recipe
                </button>
            </div>
        </form>
    </div>
